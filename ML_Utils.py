...
def makePlots(s, mlMeshShape, y0Plane_WT_idx, zhPlane_WT_idx, 
              ...
              TITestTrue, TITestPred, TIDiff, TextnLegend=True, nWTs=1):
    ...
    xWT = [0, 64, 128, 192, 256, 320]
    for i in range(12):
        for WT in range(nWTs):
            x = 7 + xWT[WT]
            if i <6:
                rect1 = Rectangle((x, 23), 1.5, 12, edgecolor='none', facecolor='w')
                rect2 = Rectangle((x, 29), 5, 1, edgecolor='none', facecolor='w')
                rect3 = Rectangle((x+2.5, 29), 1.5, 13, edgecolor='none', facecolor='w')
                ax[i].add_patch(rect1); ax[i].add_patch(rect2); ax[i].add_patch(rect3)
            else:
                rect1 = Rectangle((x, 9), 1.5, 7, edgecolor='none', facecolor='w')
                ax[i].add_patch(rect1)
            
        # ax[i].set_xticks([])
        # ax[i].set_yticks([])
        if i not in [0,3,6,9] and TextnLegend:
...
def batchSplitData(data, trainFrac, batchSize):
    if trainFrac < 1.:
        ...
        train_data_batched = train_data.batch(1).cache().prefetch(1)
        # valid_data_batched = valid_data.batch(batchSize).cache().prefetch(1)
        valid_data_batched = valid_data.batch(1).cache().prefetch(1)
